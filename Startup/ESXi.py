...
# Version 3.10 - 2026-09-14
# Author - Burke Azbill and HOL Core Team
# Verifies ESXi hosts are online and responsive
# v3.10: HOL-2703 specific - check and enforce minimum resources (>= 6 vCPUs and >= 12 GB RAM) along with CPU topology on vsp-01.* (VSP CP) VMs when lab sku is HOL-2703 (replacing set_vsp_cp_to_medium config check).
# v3.9: If set_vsp_cp_to_medium is enabled in config.ini, check and enforce minimum resources (>= 6 vCPUs and >= 12 GB RAM) along with CPU topology on vsp-01.* VMs.
...
def main(lsf=None, standalone=False, dry_run=False):
    ...
    ##    HOL-2703 specific: confirm lab sku is HOL-2703 and ensure each vsp-01.*
    ##    (VSP CP) VM has at least 6 vCPUs and 12 GB (12288 MB) memory before power-on.
    ##
    ...
    if dry_run:
        ...
        if lsf.lab_sku == 'HOL-2703':
            lsf.write_output('  HOL-2703 lab SKU: checking vsp-01.* (VSP CP) VMs for >= 6 vCPUs and >= 12 GB RAM')
        try:
            import re
                    ...
                    try:
                        ...
                        if _vsp_vms:
                            ...
                            for _vm in _vsp_vms:
                                ...
                                _cur_cores = _hw.numCoresPerSocket if _hw else None
                                _cur_num_cpu = _hw.numCPU if _hw else None
                                _cur_mem_mb = _hw.memoryMB if _hw else None
                                _power = _vm.runtime.powerState

                                lsf.write_output(
                                    f'    {_vm.name}: numCPU={_cur_num_cpu}, memoryMB={_cur_mem_mb}, '
                                    f'numCoresPerSocket={_cur_cores}, state={_power}'
                                )

                                # Determine target vCPU count and memory
                                _target_num_cpu = _cur_num_cpu
                                _target_mem_mb = _cur_mem_mb
                                if lsf.lab_sku == 'HOL-2703':
                                    if _cur_num_cpu and _cur_num_cpu < 6:
                                        _target_num_cpu = 6
                                    if _cur_mem_mb and _cur_mem_mb < 12288:
                                        _target_mem_mb = 12288
                                ...
                                _needs_cpu = (_cur_num_cpu != _target_num_cpu)
                                _needs_mem = (_cur_mem_mb != _target_mem_mb)
                                _needs_cores = (_cur_cores != _target_cores)

                                if not _needs_cpu and not _needs_mem and not _needs_cores:
                                    lsf.write_output(
                                        f'    {_vm.name}: numCPU={_cur_num_cpu}, memoryMB={_cur_mem_mb}, numCoresPerSocket={_cur_cores} - no changes needed'
                                    )
                                    continue
                                ...
                                if _power == 'poweredOn':
                                    _skip_reasons = []
                                    if _needs_cpu:
                                        _skip_reasons.append(f'numCPU: {_cur_num_cpu} -> {_target_num_cpu}')
                                    if _needs_mem:
                                        _skip_reasons.append(f'memoryMB: {_cur_mem_mb} -> {_target_mem_mb}')
                                    if _needs_cores:
                                        _skip_reasons.append(f'numCoresPerSocket: {_cur_cores} -> {_target_cores}')
                                    lsf.write_output(
                                        f'    {_vm.name}: WARNING - VM is powered on; '
                                        f'reconfiguration ({", ".join(_skip_reasons)}) skipped to avoid disruption'
                                    )
                                    _host_vsp_skipped += 1
                                    continue

                                # Powered off — safe to reconfigure vCPU, memory, and topology in a single spec
                                _spec = vim.vm.ConfigSpec()
                                _reconfig_msg_parts = []
                                if _needs_cpu:
                                    _spec.numCPUs = _target_num_cpu
                                    _reconfig_msg_parts.append(f'numCPU: {_cur_num_cpu} -> {_target_num_cpu}')
                                if _needs_mem:
                                    _spec.memoryMB = _target_mem_mb
                                    _reconfig_msg_parts.append(f'memoryMB: {_cur_mem_mb} -> {_target_mem_mb}')
                                if _needs_cores:
                                    _spec.numCoresPerSocket = _target_cores
                                    _reconfig_msg_parts.append(f'numCoresPerSocket: {_cur_cores} -> {_target_cores}')
